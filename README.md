...
Windows 专用拟稿助手：打开微信会话，按 `Ctrl+Alt+W`（或在窗口里粘贴对话），生成 2–3 条草稿，**只辅助拟稿，不会自动发送**。
...
3. 打开电脑微信，进入要回的会话，按 `Ctrl+Alt+W`。默认只 OCR 当前窗口聊天区，**不读剪贴板**。若要读剪贴板，在「设置 → 快捷键读取」里改。
4. 在夜班台点一条草稿即复制，粘回微信发送。

可选：在「设置」里填写 API Key 和接口地址。不填则走内置规则拟稿。密钥只写在本机 `%APPDATA%\夜班台\user.yaml`，不会打进安装包。

草稿库也在 `%APPDATA%\夜班台\`。设置页可以关闭聊天原文或末帧截图保存，也可以一键清除草稿、日志、快捷键去重记录和截图；清理时会保留用户设置与 API Key。卸载程序不会自动删除这份数据。
